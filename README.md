...
- **AI 요약**: `claude` CLI가 PATH에 있으면 자동 사용. 끄려면 환경변수 `CLAUDE_NOTIFY_NO_AI=1`. 요약용 하위 세션의 hook 재귀는 `CLAUDE_NOTIFY_SUPPRESS` 가드로 차단됨.- **Linux**: 트레이에 `libayatana-appindicator`, 알림에 `notify-send`(libnotify) 필요.
