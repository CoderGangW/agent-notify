...
func glyph(cx, cy, s, r float64) string {
	...
	seg(p(6.8, 8.8), p(9.8, 12))  // chevron upper arm (small)
	seg(p(9.8, 12), p(6.8, 15.2)) // chevron lower arm
	seg(p(16.2, 12), p(20.8, 12)) // dash: long, with clear air after the chevron
	return b.String()
}
...
func markSVG(color string, withWaves bool) string {
	...
	d := lucideBell(lineR, bs) + glyph(11.3, 12+(10.2-12)*bs, 0.52*bs, lineR)
	if withWaves {
		d += waves(lineR, bs)
	}
